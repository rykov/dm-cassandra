# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{dm-cassandra-adapter}
  s.version = "0.0.1"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Michael Rykov"]
  s.date = %q{2010-12-19}
  s.description = %q{Cassandra Adapter for DataMapper}
  s.email = %q{mrykov [a] gmail [d] com}
  s.extra_rdoc_files = [
    "LICENSE"
  ]
  s.files = [
    "LICENSE",
    "Rakefile",
    "VERSION",
    "conf/storage-conf.xml",
    "lib/dm-cassandra-adapter.rb",
    "lib/dm-cassandra-adapter/adapter.rb",
    "lib/dm-cassandra-adapter/spec/setup.rb",
    "spec/fixtures/book.rb",
    "spec/fixtures/difficult_book.rb",
    "spec/rcov.opts",
    "spec/semipublic/cassandra_adapter_spec.rb",
    "spec/spec.opts",
    "spec/spec_helper.rb",
    "tasks/spec.rake"
  ]
  s.has_rdoc = %q{yard}
  s.homepage = %q{http://github.com/rykov/dm-cassandra-adapter}
  s.require_paths = ["lib"]
  s.rubyforge_project = %q{datamapper}
  s.rubygems_version = %q{1.3.7}
  s.summary = %q{Cassandra Adapter for DataMapper}
  s.test_files = [
    "spec/fixtures/book.rb",
    "spec/fixtures/difficult_book.rb",
    "spec/semipublic/cassandra_adapter_spec.rb",
    "spec/spec_helper.rb"
  ]

  if s.respond_to? :specification_version then
    current_version = Gem::Specification::CURRENT_SPECIFICATION_VERSION
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<dm-core>, ["~> 1.0.2"])
      s.add_runtime_dependency(%q<dm-serializer>, ["~> 1.0.2"])
      s.add_development_dependency(%q<rspec>, ["~> 1.3"])
      s.add_development_dependency(%q<dm-validations>, ["~> 1.0.2"])
      s.add_development_dependency(%q<fakeweb>, ["~> 1.3"])
    else
      s.add_dependency(%q<dm-core>, ["~> 1.0.2"])
      s.add_dependency(%q<dm-serializer>, ["~> 1.0.2"])
      s.add_dependency(%q<rspec>, ["~> 1.3"])
      s.add_dependency(%q<dm-validations>, ["~> 1.0.2"])
      s.add_dependency(%q<fakeweb>, ["~> 1.3"])
    end
  else
    s.add_dependency(%q<dm-core>, ["~> 1.0.2"])
    s.add_dependency(%q<dm-serializer>, ["~> 1.0.2"])
    s.add_dependency(%q<rspec>, ["~> 1.3"])
    s.add_dependency(%q<dm-validations>, ["~> 1.0.2"])
    s.add_dependency(%q<fakeweb>, ["~> 1.3"])
  end
end

